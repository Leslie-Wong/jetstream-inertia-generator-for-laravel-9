...
class ColumnTransition extends ViewGenerator {
     ...
     * @var string
     */
    protected $name = 'jig:generate:column-transition';
     ...
     * @var string
     */
    protected $description = 'Add column name to json file';
    ...
    public function handle()
    {
        ...
        $name = $this->readColumnsFromTable($this->tableName)->filter(function($column) use (&$newLangStr) {
            // Log::info("columnsToQuery ==> \n".var_export($column,true));label
            $newLangStr[$column['label']] = $column['label'];
            Log::info("label ==> \n".var_export( $newLangStr,true));
        ...
        foreach (glob(resource_path('lang/*.json')) as $filename) {
            $langFiles[] = [
                "name" => explode("lang/", $filename)[1],
                "data" => array_merge(json_decode(file_get_contents($filename), true), $newLangStr)
            ];
            file_put_contents($filename, json_encode(array_merge(json_decode(file_get_contents($filename), true), $newLangStr),  JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        }



        Log::info("name array " . json_encode($langFiles) . "\n");
